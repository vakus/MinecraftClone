...
private:
    ...
    void createCommandBuffers()
    {
        ...
        for (size_t x = 0; x < commandBuffers.size(); x++)
        {
            ...
            vkCmdDrawIndexed(commandBuffers[x], static_cast<uint32_t>(indicies.size()), 1, 0, 0, 0);
    ...
    void createImageViews()
    {
        ...
        for (size_t x = 0; x < swapChainImages.size(); x++)
        {
            swapChainImageViews[x] = createImageView(swapChainImages[x], swapChainImageFormat);
        }
    }
